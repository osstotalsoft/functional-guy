...
import { stopOnFirstFailure, concatFailure, error, field, shape, items, allProps } from '../combinators'
...
describe("combinators tests:", () => {
    ...
    it("items: ", () => {
        const validator = required |> items
        expect(validator([null, null])).toStrictEqual(Failure(["0: required", "1: required"]))
        expect(validator(["null", null])).toStrictEqual(Failure(["1: required"]))
    })

    it("allProps: ", () => {
        const validator = required |> allProps
        expect(validator({name: null, surname: null, age: null})).toStrictEqual(Failure(["name: required", "surname: required", "age: required"]))
    })
